...
save_path="qq_gate_0520_"+args.model_type+"_epoch_"+str(args.epochs)+".bin"
...
if args.model_type=='gector':
    train_src_file='./dataset/qq_train_lt_gector_src.txt'
    train_trg_file='./dataset/qq_train_lt_gector_trg.txt'
elif args.model_type=='bart':
    train_src_file='./dataset/qq_train_lt_bart_src.txt'
    train_trg_file='./dataset/qq_train_lt_bart_trg.txt'
elif args.model_type=='t5':
    train_src_file='./dataset/qq_train_lt_t5_src.txt'
    train_trg_file='./dataset/qq_train_lt_t5_trg.txt'

with open(train_src_file, 'r', encoding='utf-8') as file:
    train_src = file.readlines()
with open(train_trg_file, 'r', encoding='utf-8') as file:
    train_trg = file.readlines()

train_labels = [1] * int(len(train_src)/2) + [0] *int(len(train_src)/2)
assert len(train_labels) == len(train_src) == len(train_trg)

src_np = np.array(train_src)
trg_np =np.array(train_trg)
labels_np = np.array(train_labels)

random_indices = np.random.permutation(len(train_labels))

src_np = src_np[random_indices]
trg_np = trg_np[random_indices]
labels_np =labels_np[random_indices]

train_src = src_np.tolist()
train_trg = trg_np.tolist()
train_labels= labels_np.tolist()


inputs = tokenizer(train_src, train_trg, padding=True, truncation=True, return_tensors="pt")
