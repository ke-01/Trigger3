...
def evaluate_edits(src, hyp_dict, ref_dict, best, sent_id, args):
    ...
    for hyp_id in hyp_dict.keys():
        for ref_id in ref_dict.keys():
            tp, fp, fn, _ = compareEdits(hyp_dict[hyp_id], ref_dict[ref_id])
            # for tp
            if tp>0 or (tp == 0 and fp == 0 and fn == 0): 
                tp_neg.append(sent_id)
            else:
                tp_pos.append(sent_id)
            
            # for fp
            if fp>0:
                fp_pos.append(sent_id)
            else:
                fp_neg.append(sent_id)
            
            # for fn
            if fn>0:
                fn_pos.append(sent_id)
            else:
                fn_neg.append(sent_id)
...
def print_results(best, best_cats, args):
    ...
    # white to
    tp_pos_file='./checker_file/'+str(args.file_name)+'tp_pos.json'
    tp_neg_file='./checker_file/'+str(args.file_name)+'tp_neg.json'
    
    fp_pos_file='./checker_file/'+str(args.file_name)+'fp_pos.json'
    fp_neg_file='./checker_file/'+str(args.file_name)+'fp_neg.json'
    
    fn_pos_file='./checker_file/'+str(args.file_name)+'fn_pos.json'
    fn_neg_file='./checker_file/'+str(args.file_name)+'fn_neg.json'
    
    save_list_to_file(tp_pos, tp_pos_file)
    save_list_to_file(tp_neg, tp_neg_file)
    save_list_to_file(fp_pos, fp_pos_file)
    save_list_to_file(fp_neg, fp_neg_file)
    save_list_to_file(fn_pos, fn_pos_file)
    save_list_to_file(fn_neg, fn_neg_file)
...
if __name__ == "__main__":
    # Run the program
    main()
    print('pos tp:{} fp:{} fn:{}'.format(len(tp_pos),len(fp_pos),len(fn_pos)))
    print('neg tp:{} fp:{} fn:{}'.format(len(tp_neg),len(fp_neg),len(fn_neg)))
